...
-- Частицы на тележке (создаются лениво при первом кадре)
...
-- Запрещённые зоны (квадраты, внутрь которых бот не заходит)
...
samp.onServerMessage = function(color, text)
    ...
    if cVARS.bot[0] and not text:find(u8("\xE3\xEE\xE2\xEE\xF0\xE8\xF2")) and not text:find(u8("\xEA\xF0\xE8\xF7\xE8\xF2")) then
        ...
        elseif text:find(u8("\xCF\xEE\xE4\xEE\xE6\xE4\xE8\xF2\xE5, \xE4\xF0\xF3\xE3\xEE\xE9 \xE8\xE3\xF0\xEE\xEA \xF1\xE5\xE9\xF7\xE0\xF1 \xF0\xF3\xE1\xE8\xF2 \xE4\xE5\xF0\xE5\xE2\xEE")) then
            -- Дерево занято другим игроком – добавляем в игнор и ищем другое
            resetNavPath()
    ...
    -- Групповой бонус: "[Группа] Участник группы доставил дрова! Ваш бонус: +NNN дров, + MM.MMM"
    do
...
function onReceivePacket(id, bs) 
    if id == 220 then
        ...
        if subId == 17 then
            ...
            -- ================= НАЧАЛО: УЧЁТ ДРОВ, ДРЕВЕСИНЫ И ДЕНЕГ =================
            ...
            if str:find("event.player.addMoney") and sawmill_hidden then
                ...
                sawmill_hidden = false -- сброс флага сразу после проверки
                ...
                if money_amt > 0 then
                    ...
                    -- Формула с учётом прямой выплаты денег
            ...
            -- ================= КОНЕЦ: УЧЁТ ДРОВ, ДРЕВЕСИНЫ И ДЕНЕГ =================
...
imgui.OnFrame(function() return cVARS.menu[0] end,
    function(self)
        ...
        -- РјРёРіР°СЋС‰Р°СЏ РєРЅРѕРїРєР° РїРѕРґРїРёСЃРєРё РЅР° С‚РіРє
