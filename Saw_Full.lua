...
local nav_movement = require("navmesh.movement")
local nav_render = require("navmesh.render")
...
function playAlertSound()
    ...
    if alert_audio then
        ...
        cMsg('Ошибка загрузки звука антиадмина: ' .. path)
    end
end
...
local function reload_avatar()
    ...
    if doesFileExist(full_path) then
        ...
        if tex then
            avatar_texture = tex
            print("NIG | Аватар успешно загружен: " .. full_path)
        else
            sampAddChatMessage("[CConsole] Не удалось загрузить аватар: " .. full_path, 0xFF4444)
        end
    else
        sampAddChatMessage("[CConsole] Файл аватара не найден: " .. full_path, 0xFF4444)
    end
end
    ...
    while true do wait(0)
        ...
        if cVARS.bot[0] then
            ...
            elseif bot_state == "GO_KYSHAT" then
                navRunToPoint(-537.37, -193.53, 79.52, true)
                local distance = distPoint(-537.37, -193.53, 79.52)
            ...
            elseif bot_state == "SEARCH_TREE" then
                ...
                if f then
                    ...
                    if has_drova and not has_telega then
                        ...
                        -- ждём showModal от сервера, showModal сам переключит в PLAY_MINIGAME
                    end
                else
            ...
            elseif bot_state == "GO_TO_TREE_POINT" then
                ...
                if dist_to_target < 4.0 then
                    ...
                    if dist_to_target < 1.65 then
                        -- ждём showModal от сервера, он сам переключит в PLAY_MINIGAME
...
function runToPoint(x, y, z, cam_x, cam_y, cam_z)
    ...
    local should_walk = near_tree or near_sdacha
...
function isTreeLabel(text)
    return text and text:find("Дерево") and (text:find("Доступно") or text:find("надрублено"))
end
...
function hasDrovaLabelNear(x, y, z, radius)
    ...
    for id = 0, 2048 do
        if sampIs3dTextDefined(id) then
            local text, color, posX, posY, posZ = sampGet3dTextInfoById(id)
            if text and text:find("Дрова:") then
                local dist = getDistanceBetweenCoords3d(posX, posY, posZ, x, y, z)
                if dist < radius then
...
--самп ивентс
function samp.onApplyPlayerAnimation(player_id, anim_lib, anim_name, loop, lock_x, lock_y, freeze, time) -- мб понадобится, потом посмтрю
end
...
otvet = false
local answerWords = {'вы тут?','Вы тут?'}
local otvet_1 = {
    "/b тута я тута",
    "/b шо те?",
    "/b да?",
...
samp.onServerMessage = function(color, text)
    ...
    if text:find("Заберите тележку с дровами рядом с деревом!") then
        local mX, mY, mZ = getCharCoordinates(PLAYER_PED)
        local found = false
        for id = 0, 2048 do
            if sampIs3dTextDefined(id) then
        ...
        if not found then
            -- метка "Дрова:" ещё не появилась — ждём через need_find_telega
            if current_tree_points and #current_tree_points > 0 then
                saved_tree_points = current_tree_points
                saved_tree_point_idx = current_tree_point_idx or 1
            end
    ...
    if cVARS.bot[0] and not text:find("говорит") and not text:find("кричит") then
        if text:find("Вы слишком далеко от дерева!") then
            resetNavPath()
            bot_state = "SEARCH_TREE"
        elseif text:find("Тележка с дровами была потеряна") and not (bot_state == "ESCAPE_VEHICLE") then
            resetNavPath()
            bot_state = "SEARCH_TREE"
        end
    end
    local text, prefix, color = sampGetChatString(99)
    local filename = text:match("Скриншот сохранен: (%d+%.%d+%.%d+%.%d+%.jpg)")
        ...
        if subId == 17 then
            ...
            if cVARS.bot[0] and str:find("LumberingGame", 1, true) and str:find("showModal", 1, true) then
                if bot_state == "GO_TO_TREE_POINT" or bot_state == "SEARCH_TREE" then
                    -- сервер открыл нашу минигру — переходим в PLAY_MINIGAME
                    set_wait_alt = os.clock()
                    minigame_running = true
                    minigame_done_time = 0
                    ...
                    state_entered["PLAY_MINIGAME"] = false
                elseif bot_state == "PLAY_MINIGAME" then
                    -- повторный showModal = конец минигры
                    minigame_running = false
                ...
                -- любой другой bot_state = чужая минигра, игнорируем
            end
            ...
            if str:find("lumberingGame%.updateGameState") then
                ...
                if new_partner then
                    local is_absent = (new_partner:find("\xce\xf2\xf1\xf3\xf2") ~= nil)
                    if not is_absent then
                        partner_name = new_partner
                        if partner_absent then
                            partner_absent = false
                            --sampAddChatMessage("[BOT] partner returned: " .. new_partner, 0x00FF00)
                        end
                    else
                        if not partner_absent and cVARS.bot[0] and bot_state == "PLAY_MINIGAME" then
                            partner_absent = true
                            if waiting_for_my_turn then
                                --sampAddChatMessage("[BOT] partner absent while waiting -> GO_TO_TREE_POINT", 0xFFAA00)
                                sendCustomPacket("lumbering-game.exit")
                                isProcessingTurn = false
                                taskQueue = {}
                                minigame_running = false
                                ...
                                --sampAddChatMessage("[BOT] partner absent, finishing current turn first...", 0xFFAA00)
                            end
                        end
                    end
                else
            ...
            if str:find("event.arizonahud.showSawmillNotification") and
               str:find("ВАША ОЧЕРЕДЬ РЕЗАТЬ") then
                waiting_for_my_turn = false
                my_turn_ready = true
                --sampAddChatMessage("[BOT] your turn ready!", 0x00FF00)
            end
            if str:find("event.arizonahud.showSawmillNotification") and str:find("Вы получили тележку") then
                need_find_telega = true
                need_find_telega_time = os.clock()
                --sampAddChatMessage("[BOT] telega notification received, searching...", 0xFFFF00)
            end
            if str:find("event.arizonahud.showSawmillNotification") and str:find("Вы сдали дрова на переработку") then
                resetNavPath()
                ...
                elseif next_telega_pos then
                    ...
                    state_entered["PICKUP_TELEGA"] = false
                    bot_state = "PICKUP_TELEGA"
                    --sampAddChatMessage("[BOT] going for saved telega!", 0x00FF00)
                else
                    bot_state = "SEARCH_TREE"
...
function sendTelegramNotification(msg, chat_id)
    ...
    if not chat_id or chat_id == "" then
        cMsg("Ошибка: chat_id не указан или пустой. Проверьте настройки Telegram.")
        return
    end
    msg = msg:gsub('{......}', '')
    msg = encodeUrl(msg)
            ...
            for i, res in ipairs(proc_table.result) do
                if res.update_id > updateid then
                    ...
                    if message and message.text then
                        ...
                        elseif text == "Краш игры" then
                            piska = true
                            sendTelegramNotification("Игра крашнута!", chat_id)
                        elseif text == "Сделать скриншот" then
                            lua_thread.create(function()
                                cumshot = true
                                wait(1000)
...
function GraySeparator(thickness, padding)
    local dl = imgui.GetWindowDrawList()
    local p = imgui.GetCursorScreenPos()
    local width = imgui.GetWindowSize().x - 10
...
imgui.OnFrame(function() return cVARS.menu[0] end,
    function(self)
        ...
        imgui.Begin(u8"LesoРуб бот##main", cVARS.menu, imgui.WindowFlags.NoTitleBar)
        ...
        local scroll_text = u8"Подпишись пж на мой тгк "
        ...
        dl:PopClipRect()
        imgui.PopFont()
        local tab_x_start = scroll_clip_end +370
        ...
        imgui.PopFont()
        local buttons = {
            {name = u8"Главная",     icon = ti.ICON_TREES},
            {name = u8"Сеттинг",  icon = ti.ICON_SETTINGS},
            {name = u8"Telegram",    icon = ti.ICON_BRAND_TELEGRAM},
            {name = u8"Антиадм",   icon = ti.ICON_SHIELD_OFF},
            {name = u8"Анти-Еда",    icon = ti.ICON_TOOLS_KITCHEN_2},
            {name = u8"Антибот",     icon = ti.ICON_ROBOT_OFF},
            {name = u8"Стата",  icon = ti.ICON_PRESENTATION},
            {name = u8"Гайды",  icon = ti.ICON_BOOK_2}
        }
        ...
        if imgui.Button(u8"Перейти в тгк", iv2(block_width - 40, 28)) then
        ...
        imgui.PopFont()
        imgui.Spacing()
        local telegram_popup_open = new.bool(true)
        ...
        imgui.PushFont(fonts[16])
        if selected_tab == u8"Главная" then

            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.ToggleButton(ti.ICON_TREES .. u8" Бот дерева", cVARS.bot, 0.15)
            ...
            imgui.SliderInt(ti.ICON_RULER_2 .. u8" Размер радара", cVARS.radar_size, 100, 500)
            imgui.PopItemWidth()
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(math.min(300))
            imgui.SliderInt(ti.ICON_RULER_2 .. u8" Зум радара", cVARS.radar_zoom, 5, 100)
            imgui.PopItemWidth()
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(50)
            if clp(u8'Доп.функции',480) then
                ...
                if imgui.BeginPopupModal("##object_popup", object_popup_open, imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoResize + imgui.WindowFlags.AlwaysAutoResize) then
                    ...
                    imgui.ToggleButton(u8" На все объекты", object, 0.15)
                    imgui.ToggleButton(u8" Только на пни", objectpen, 0.15)
                    imgui.ToggleButton(u8" Не включать на деревья", objectnotree, 0.15)
                ...
                if cVARS.animspeed_enabled[0] then
                    ...
                    imgui.SliderFloat(u8"Скорость анимаций", cVARS.animspeed_value, 0.1, 10.0)
                    imgui.PopItemWidth()
                end
                imgui.SameLine()
                imgui.SetCursorPosX(pyramid_cursor_x1(130))
                imgui.ToggleButton(u8"На машины", vehicle, 0.15)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.ToggleButton(u8"Беспалевный бег CJ", cVARS.runskincj, 0.15)
                imgui.SameLine() imgui.ShowHelpMarker(u8"Выдаст бег сиджея на любой скин")
                imgui.SameLine()
                imgui.SetCursorPosX(pyramid_cursor_x1(140))
                imgui.ToggleButton(u8" На игроков", player, 0.15)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
            ...
            imgui.PopItemWidth()
        elseif selected_tab == u8"Сеттинг" then

            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(168)
            if imgui.InputTextWithHint("##menu_command", u8"Команда меню", cVARS.menu_command, 64) then
                config.menu_command = ffi.string(cVARS.menu_command)
                save_cfg()
            end
            if imgui.IsItemHovered() then
                imgui.SetTooltip(u8"Команда открытия меню")
            end
            imgui.PopItemWidth()
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(220)
            if imgui.InputTextWithHint("##bot_command", u8"Команда бота", cVARS.bot_command, 64) then
                config.bot_command = ffi.string(cVARS.bot_command)
                save_cfg()
            end
            if imgui.IsItemHovered() then
                imgui.SetTooltip(u8"Команда для запуска бота")
            end
            imgui.PopItemWidth()
            ...
            local combo1_text = u8"Выбрать 1 клавишу"
            if cVARS.waiting_for_combo_key1 then
                combo1_text = u8"Нажмите 1 клавишу..."
                local key = getPressedKey()
                if key then
            ...
            imgui.Text("+")
            imgui.SameLine()
            local combo2_text = u8"Выбрать 2 клавишу"
            if cVARS.waiting_for_combo_key2 then
                combo2_text = u8"Нажмите 2 клавишу..."
                local key = getPressedKey()
                if key then
            ...
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.Text(u8"Путь к аватару (папка moonloader):")
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(350)
            if imgui.InputTextWithHint("##avatar_path", u8"например: avatar.png", cVARS.avatar_path, 256) then
        ...
        elseif selected_tab == u8"Telegram" then
            ...
            imgui.PopItemWidth()
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.Text(u8"Прокси / резервный URL API Telegram:")
            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.PushItemWidth(500)
            ...
            imgui.TextWrapped(u8"Публичные прокси: https://tg.bakh.us  |  https://tgapi.ru")
            imgui.PopStyleColor()
            imgui.SetCursorPosX(pyramid_cursor_x())
            if imgui.Button(u8"Сбросить на стандартный", iv2(280, 30)) then
        ...
        elseif selected_tab == u8"Антиадм" then
        ...
        elseif selected_tab == u8"Анти-Еда" then

            imgui.SetCursorPosX(pyramid_cursor_x())
            if clp(u8'Автоеда',170) then
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                ...
                if cVARS.autoeat[0] then
                    ...
                    if imgui.Combo(u8'Способ еды', cVARS.eatmethod, ImItems, #method) then config.eatmethod = cVARS.eatmethod[0] save_cfg() end
                    imgui.PopItemWidth()
                    imgui.SetCursorPosX(pyramid_cursor_x(10))
        ...
        elseif selected_tab == u8"Антибот" then
            ...
            imgui.SameLine()
            imgui.ShowHelpMarker(u8"Если бот застрнет, прыгнет и пришёт увед, дальше дело за вами")

            imgui.SetCursorPosX(pyramid_cursor_x())
            imgui.ToggleButton(u8" Рандомные повороты", cVARS.enable_random_turns, 0.15)

            imgui.SetCursorPosX(pyramid_cursor_x())
            if imgui.ToggleButton(u8" Показать NavMesh сетку", cVARS.show_navmesh, 0.15) then
                config.show_navmesh = cVARS.show_navmesh[0]
                save_cfg()
            end
            imgui.SameLine() imgui.ShowHelpMarker(u8"Отображает сетку навигации в игровом мире")
            imgui.SetCursorPosX(pyramid_cursor_x())
            ...
            if not cVARS.smart_sdacha[0] then
                ...
                if imgui.Combo(u8"Ручной выбор точки", cVARS.selected_sdacha_index, ImItems_sdacha, #sdacha_names) then
            ...
            imgui.SetCursorPosX(pyramid_cursor_x())
            if clp(u8'Настройки камеры',640) then
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.PushItemWidth(250)
                imgui.SliderFloat(u8"Сглаживание ближнее (<8м)", cVARS.camera_smooth_close, 0.01, 0.5)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Сглаживание среднее (<25м)", cVARS.camera_smooth_mid, 0.01, 0.5)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Сглаживание дальнее", cVARS.camera_smooth_far, 0.01, 0.5)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Поворот плавный (<20°) °/с", cVARS.camera_turn_slow, 5.0, 180.0)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Поворот средний (<90°) °/с", cVARS.camera_turn_mid, 30.0, 360.0)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Поворот резкий (>90°) °/с", cVARS.camera_turn_fast, 60.0, 720.0)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Дистанция камеры", cVARS.camera_dist, 5.0, 20.0)
                imgui.SetCursorPosX(pyramid_cursor_x(10))
                imgui.SliderFloat(u8"Высота камеры", cVARS.camera_height_offset, 0.0, 3.0)
                imgui.PopItemWidth()
            end
            ...
            if imgui.Button(u8"Сохранить настройки", iv2(300, 40)) then
                ...
                config.enable_random_pauses = cVARS.enable_random_pauses[0]
                save_cfg()
                sampAddChatMessage("Настройки антибота сохранены!", -1)
            end

        elseif selected_tab == u8"Стата" then
            local function DrawStatBlock(title, icon, day_value, week_value, pos_offset_x, pos_offset_y, bg_color, border_color, width, height, text_offset)
                ...
                imgui.BeginGroup()
                    ...
                    imgui.Text(u8"    За день:   " .. tostring(day_value))
                    imgui.SetCursorPosX(value_x)
                    imgui.Text(u8"    За неделю: " .. tostring(week_value))
            ...
            imgui.BeginGroup()
                imgui.SetCursorPosX(pyramid_cursor_x())
                imgui.ToggleButton(ti.ICON_PRESENTATION .. u8" Открыть стату", cVARS.stat_status, 0.15)
                imgui.SetCursorPosX(pyramid_cursor_x())
                imgui.ToggleButton(ti.ICON_RAINBOW .. u8" Радужный текст", cVARS.rainbowcolor, 0.15)
                imgui.SetCursorPosX(pyramid_cursor_x())
                imgui.PushItemWidth(150)
                imgui.InputInt(u8"Цена за дерево $", cVARS.derevo_value)
                imgui.SetCursorPosX(pyramid_cursor_x())
                imgui.InputInt(u8"Цена за дрова $", cVARS.drova_value)
                imgui.PopItemWidth()
                imgui.SetCursorPosX(pyramid_cursor_x())
            ...
            imgui.EndGroup()
            local rashetzalypi = cVARS.derevo_amount[0] * cVARS.derevo_value[0] + cVARS.drova_amount[0] * cVARS.drova_value[0]
            if rashetzalypi>0 then
                cVARS.degniebat_den[0] = rashetzalypi
                cVARS.degniebat_week[0] = rashetzalypi
                save_cfg()
            end
            DrawStatBlock(u8"Деньги", ti.ICON_WOOD, 
                        ...
                        360, 155,
                        10)
            DrawStatBlock(u8"Дерево высшего качества", ti.ICON_WOOD,
                        ...
                        40)
            imgui.SameLine()
            DrawStatBlock(u8"Дрова", ti.ICON_WOOD,
...
Включите бота на вкладке "Главная"
Настройте антиадмин-функции для безопасности
...
По всем вопросам: @Official_Peredoz
и абазятально оформи падпиську на мой тгк: @flupiflufi
            ]])
        end
